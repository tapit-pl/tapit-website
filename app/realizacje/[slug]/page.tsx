  ...
  return (
    <main className="pt-12 pb-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <Link
          href="/realizacje"
          className="inline-flex items-center gap-2 text-sm text-[#6b6860] hover:text-accent transition-colors mb-8"
        >
          <ArrowLeft size={15} />
        ...
        <div className="mb-10">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-5">
            <span className="text-xs font-medium px-3 py-1 rounded-full bg-[rgba(0,0,0,0.06)] border border-[rgba(0,0,0,0.09)] text-[#6b6860]">
              {cs.industryTag}
            </span>
          ...
          <p className="text-[#6b6860] text-lg">{cs.industry}</p>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {cs.services.map(s => (
              <span
                key={s}
                className={`text-xs font-medium px-3 py-1 rounded-full border ${serviceColors[s] ?? 'bg-[rgba(0,0,0,0.06)] text-[#6b6860] border-[rgba(0,0,0,0.09)]'}`}
              >
                {s}
              </span>
            ))}
        ...
        {/* Results hero */}
        <div className={`rounded-2xl bg-gradient-to-br ${cs.coverColor} border border-[rgba(0,0,0,0.09)] p-8 mb-12`}>
          <p className="text-xs uppercase tracking-[2px] text-[#6b6860] font-heading font-bold mb-5">
            Kluczowe wyniki
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {cs.results.map(r => (
              <div key={r.label}>
                ...
                <p className="font-medium text-[#1c1b19] text-sm mb-1">{r.label}</p>
                <p className="text-xs text-[#6b6860] leading-relaxed">{r.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <section className="mb-10">
          <h2 className="font-heading font-bold text-xl text-[#1c1b19] mb-4 flex items-center gap-3">
          ...
          <p className="text-[#6b6860] leading-relaxed text-[15px]">{cs.challenge}</p>
        </section>

        <div className="border-t border-[rgba(0,0,0,0.07)] mb-10" />
        ...
        <section className="mb-10">
          ...
          <p className="text-[#6b6860] leading-relaxed text-[15px] mb-5">{cs.solution}</p>

          <div className="space-y-2.5">
            {cs.solutionPoints.map((point, i) => (
              <div key={i} className="flex items-start gap-3 p-3.5 rounded-xl bg-[rgba(0,0,0,0.03)] border border-[rgba(0,0,0,0.06)]">
                <CheckCircle size={15} className="text-success shrink-0 mt-0.5" />
                <p className="text-sm text-[#6b6860]">{point}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-12">
          <h2 className="font-heading font-bold text-xl text-[#1c1b19] mb-6 flex items-center gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {cs.results.map(r => (
              <div key={r.label} className="p-5 rounded-2xl bg-[rgba(0,0,0,0.04)] border border-[rgba(0,0,0,0.08)] text-center">
                <p className="font-heading font-bold text-3xl text-accent mb-1">{r.metric}</p>
                <p className="font-medium text-[#1c1b19] text-sm mb-2">{r.label}</p>
                <p className="text-xs text-[#6b6860] leading-relaxed">{r.desc}</p>
              </div>
            ))}
          </div>
        ...
        <blockquote className="relative p-8 rounded-2xl bg-[rgba(0,0,0,0.04)] border border-[rgba(0,0,0,0.08)] mb-16">
          ...
          <div className="flex items-center gap-3 border-t border-[rgba(0,0,0,0.07)] pt-4">
            ...
            <div>
              <p className="font-medium text-[#1c1b19] text-sm">{cs.quoteName}</p>
              <p className="text-xs text-[#6b6860]">{cs.quoteRole}</p>
            </div>
          </div>
        </blockquote>
        ...
        <div className="text-center p-10 rounded-3xl bg-gradient-to-br from-accent/10 to-transparent border border-accent/20 mb-16">
          ...
          <p className="text-[#6b6860] mb-6 max-w-sm mx-auto">
        ...
        {otherCases.length > 0 && (
          <section>
            <h3 className="font-heading font-bold text-lg text-[#1c1b19] mb-5">
              Inne realizacje
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {otherCases.map(c => (
                <Link
                  ...
                  className="group p-4 rounded-xl bg-[rgba(0,0,0,0.04)] border border-[rgba(0,0,0,0.08)] hover:border-accent/30 transition-all"
                >
                  <div className={`h-1 rounded-full bg-gradient-to-r ${c.coverColor} mb-3`} />
                  <p className="font-medium text-[#1c1b19] text-sm group-hover:text-accent transition-colors mb-1">{c.client}</p>
                  <p className="text-xs text-[#6b6860]">{c.industryTag}</p>
